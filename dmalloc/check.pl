...
sub run_compare_print_actual ($$$) {
    ...
    _print("  $Redctx", $apfx, ": Got `", $context, "`$Off\n");
}
...
sub run_compare ($$$$$$) {
    ...
    for (; $a != @$actual; ++$a) {
        ...
        if (!@explines && !$exp->{skip}) {
            ...
            _print("$Red${outname}FAIL: Too much output (expected ", $exp->{nl}, " output $lines)$Off\n");
        ...
        if ($ok) {
            ...
            _print("$Red${outname}FAIL: Unexpected output starting on line ", $a + 1, "$Off\n");
            _print("  $Redctx$ename:", $explines[0]->{line}, ": Expected `", $explines[0]->{t}, "`\n");
            run_compare_print_actual($actual, $a, $aname);
	    return 1;
        }
    }
    ...
    if (@explines) {
        _print("$Red${outname}FAIL: Missing output starting on line ", scalar(@$actual), "$Off\n");
        _print("  $Redctx$ename:", $explines[0]->{line}, ": Expected `", $explines[0]->{t}, "`$Off\n");
        return 1;
    } else {
        my($ctx) = "";
        if ($exp->{time}) {
        ...
        _print("$Green${outname}OK$ctx$Off\n");
        return 0;
    }
}
...
sub asan_options ($) {
    ...
    # fix for F2025: don't specify what to do about sanitizers bc Makefile
    # handles that
...
if ($Exec) {
	...
	push @test_results, {
	    name  => $test_name,
	    status => ($res == 0) ? "passed" : "failed",
	    output => _collect(),
	};
    }
    my($ntestpassed) = $ntest - $ntestfailed;
    if ($ntest == $maxtest && $ntestpassed == $ntest) {
        print("${Green}All tests passed!$Off\n");
    } else {
        my($color) = ($ntestpassed == 0 ? $Red : ($ntestpassed == $ntest ? $Green : $Cyan));
        _print("${color}$ntestpassed of $ntest ", ($ntest == 1 ? "test" : "tests"), " passed$Off\n");
    }

    if ($json_file) {
	open(my $fd, ">", $json_file) or die "Could not open ${json_file}:  $!";
	my $json = JSON::PP->new->utf8->pretty;
	print $fd $json->encode({ tests => \@test_results });
    }
}
