...
int main(int argc, char* argv[]) {
    ...
    struct io300_file* in = io300_open(argv[2], MODE_READ, "in");
    if (in == NULL) {
        return 1;
    }

    struct io300_file* out = io300_open(argv[3], MODE_WRITE, "out");
    if (out == NULL) {
        io300_close(in);
        return 1;
    }
    ...
    if (filesize <= 0) {
        fprintf(stderr, "error: could not get filesize on empty file\n");
        io300_close(in);
        io300_close(out);
        return 1;
    }
