...
std::pair<Shard, Shard> split_shard(const Shard& shard) {
  ...
  // get new bounds for each shard (upper bound of lower shard, and lower
  // bound of upper shard)
...
OverlapStatus get_overlap(const Shard& a, const Shard& b) {
  if (a.upper < b.lower || b.upper < a.lower) {
     ...
     * A: [-----]
     * B:         [-----]
     * Note: this also includes the flipped case (where B's keys are
     * entirely before A's)
     */
...
std::string bucket_to_str(size_t bucket, size_t granularity) {
  ...
  // Use reverse order through GRANULARITY_OPTS, as we want to find the
  // largest bucket at the current index before moving onto the next one.
  //
...
size_t str_to_bucket(const std::string& s) {
  ...
  VALID_CHARS corresponds to a bucket containing 36^(granularity - 1) keys.
  For example, if granularity = 3, the keyspace is [000, ZZZ], the total
  number of keys is 36^3, and each bucket maps to 36^2 keys.
  */
