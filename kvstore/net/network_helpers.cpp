...
int sendall(int fd, void* buf, size_t len, int flags, milliseconds timeout) {
  ...
  while (n_sent < n_to_send) {
    ...
    // update bytes sent; if not completed, network is likely saturated, so
    // wait
    n_sent += curr;
    if (n_sent < n_to_send) {
...
int open_listener_socket(const std::string& address) {
  ...
  // if ((ret = getaddrinfo(hostname.c_str(), port.c_str(), &hints, &res)) !=
  // 0)
  // {
  ...
  for (cur = res; cur != NULL; cur = cur->ai_next) {
    int yes = 1;
    // create socket w/ addr family (INET/6), type (STREAM/DGRAM), and
    // protocol (can specify IPPROTO_UDP or TCP, but here either is ok)
    ...
    // setsockopt [socket] [level (e.g. socket, or for a protocol)]
    // [OPTIONS] [OPTION VALUE] [OPTION LEN] Here, SO_REUSEADDR allows us to
    // instantly re-use port, rather than waiting for TCP TIME_WAIT to
    // expire
