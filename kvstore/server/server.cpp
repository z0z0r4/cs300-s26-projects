...
int KvServer::start() {
    ...
    // tell the shardcontroller that the server has joined, and start query
    // thread
    if (!this->shardcontroller_address.empty()) {
        ...
        // Take a look at the functions provided in this file to see if any of
        // them will help you!
        Join();
...
bool KvServer::process_config() {
    ...
    if (!res) {
        cerr_color(RED,
                   "Failed to receive query response from shardcontroller.");
        return false;
    }
    ...
    for (auto& [key, value] : all_kvpairs()) {
        auto responsible_server = new_config.get_server(key);
        if (responsible_server.has_value()) {
            if (responsible_server.value() != address) {
                to_transfer[responsible_server.value()][0].push_back(key);

                DeleteRequest req{key};
                DeleteResponse res{};
                this->store->Delete(&req, &res);

                // 选最新的 value
                to_transfer[responsible_server.value()][1].push_back(res.value);
            }
        } else {
            // 该分片不存在于任何节点
            DeleteRequest req{key};
    ...
    // TODO:
    //  1. Get all of the server's key-value pairs (which KvStore functions
...
void KvServer::work_loop(size_t worker_id) {
    // Each worker thread will run this function. While the server is not
    // stopped, pop an accepted connection off of the work queue, and process
    // client requests until the client closes the connection.
...
void KvServer::process_config_loop() {
    ...
    while (!this->is_stopped) {
        if (this->process_config()) {
            ...
            failure_count += 1;
            // process_config can fail if the destination server for a move
            // hasn't queried the shardcontroller yet so we tolerate some
...
ShardControllerConfig KvServer::get_config() { return this->config; }
