...
class KvServer {
 ...
 private:
  ...
  /**
   * Check whether this server is responsible for a key.
   *
   * WARNING: Do NOT use this member function. It is intended
   * for use in process_request only. For process_config, try using get_server
   * (common/config.hpp) directly.
   */
  bool responsible_for(const std::string& key);

  /**
   * Check whether this server is responsible for a list of keys.
   *
   * WARNING: Do NOT use this member function. It is intended
   * for use in process_request only. For process_config, try using get_server
   * (common/config.hpp) directly.
