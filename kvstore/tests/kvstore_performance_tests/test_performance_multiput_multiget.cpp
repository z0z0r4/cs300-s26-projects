...
int main(int argc, char* argv[]) {
  ...
  /*
    ...
    that the time it takes to service MultiGet and MultiPut requests
    *decreases* as the number of threads *increases*. We spawn a single thread
    that executes N_THREADS MultiPut/MultiGet requests sequentially, then
    spawn N_THREAD threads, each of which executes one request. If you've
    properly implemented bucket-based locking, the multithreaded operations
    should finish faster than the single-threaded one. We assert that the
    multithreaded operations take less than half the amount of time as the
  ...
  */
  ...
  auto insert_all = [&]() {
    for (std::size_t i = 0; i < N_THREADS; i++) {
      ...
      // values are a vector of the index (as a string) repeated
  ...
  // Define helper that threads use to MultiPut the keys into the
  // multi-threaded store. Note that since each entry in buckets_to_keys
  // touches a single bucket (that's unique from the other buckets in
  // buckets_to_keys), we are guaranteed that if you have implemented
  // bucket-based locking correctly, each MultiPut request can execute *at the
  // same time* on N_THREADS buckets.
