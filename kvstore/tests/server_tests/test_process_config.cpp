...
constexpr std::chrono::milliseconds sleepTime(1000);
static constexpr std::size_t kRandStringLength = 5;
static constexpr std::size_t kNumKeyValPairs = 100;

void ServerTest::test_process_config(std::shared_ptr<Shardcontroller> sm,
...
int ServerTest::run_test() {
  ...
  std::vector<std::string> chars_in_shard = {"01234567", "89ABCDEabcde",
                                             "FGHIJKLfghijkl", "MNOPQRSmnopqrs",
                                             "TUWXYZtuwxyz"};

  // Put some key/value pairs on the servers
  for (std::size_t i = 0; i < N_SERVERS; i++) {
    // Avoid need for client MultiPut request by only generating keys
    // comprised of chars_in_shard[i]
    keys.push_back(
