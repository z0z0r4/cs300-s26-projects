...
int main() {
  ...
  // overlap_start for server_addresses[2], no_overlap for
  // server_addresses[3])
  ...
  // server_addresses[1]): since server_addresses[1] already has this shard,
  // the config shouldn't change
  ASSERT(test_move(sm, server_addresses[1], {{"0", "7"}}));
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);
  ...
  correct_config[server_addresses[2]] = std::vector<Shard>{};
