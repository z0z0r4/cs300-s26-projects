...
int main() {
  ...
  std::vector<std::vector<std::string>> server_chunks =
  ...
  execute_in_parallel(
      [&](std::vector<std::string>&& chunk) {
        for (std::string server : chunk) {
          ASSERT(test_join(sm, server, true));
        }
      },
      server_chunks);

  for (std::vector<std::string> chunk : server_chunks) {
    for (std::string server : chunk) {
      correct_config[server] = std::vector<Shard>{};
    }
  }
  ...
  /* Shard the keyspace into N_SERVERS / N_CHUNKS shards, then concurrently
    move these shards to N_CHUNKS different servers. For example, with
    N_SERVERS = 1000 and N_CHUNKS = 10, we create server_chunks = {<100
    servers here>, <next 100 servers>, .... <10th vector of 100 servers>},
    then call split_into to make 1000/10 = 100 shards. Then, we concurrently
    iterate through each "chunk" (i.e., vector of 100 servers) and assign each
    of the 100 shards to the server at the corresponding index. In our
    example, shard #90 is moved to the 90th server in each of the 10 chunks
    concurrently. Since these moves happen concurrently, there's no guarantee
    as to which server gets each shard in the end, but if your code is
    thread-safe, all of the calls to move should succeed!
  */
  std::vector<Shard> shards = split_into(N_SERVERS / N_CHUNKS);
  execute_in_parallel(
      [&](std::vector<std::string>&& chunk) {
