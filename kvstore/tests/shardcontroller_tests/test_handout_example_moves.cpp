...
int main() {
  ...
  std::string bear_server = "bear:713";
  std::string elephant_server = "elephant:4000";
  std::string tiger_server = "tiger:9999";
  ...
  correct_config[bear_server] = std::vector<Shard>{};
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);

  ASSERT(test_join(sm, elephant_server, true));
  correct_config[elephant_server] = std::vector<Shard>{};
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);

  ASSERT(test_join(sm, tiger_server, true));
  correct_config[tiger_server] = std::vector<Shard>{};
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);

  // TABLE #2
  // Initial moves
  // Set up correct_config with shards we're moving
  correct_config[elephant_server] = std::vector<Shard>{{"0", "9"}};
  correct_config[tiger_server] = std::vector<Shard>{{"A", "G"}};
  correct_config[bear_server] = std::vector<Shard>{{"H", "L"}};
  ...
  correct_config[elephant_server] = std::vector<Shard>{{"0", "9"}, {"H", "L"}};
  ...
  correct_config[elephant_server] =
      std::vector<Shard>{{"0", "1"}, {"6", "9"}, {"H", "L"}};
  correct_config[tiger_server] = std::vector<Shard>{{"2", "5"}, {"A", "G"}};
  ASSERT(test_move(sm, tiger_server, std::vector<Shard>{{"2", "5"}}));
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);

  // TABLE #5
  // bear:713 rejoins; move [A, Z] to it
  ASSERT(test_join(sm, bear_server, true));
  correct_config[elephant_server] = std::vector<Shard>{{"0", "1"}, {"6", "9"}};
  correct_config[tiger_server] = std::vector<Shard>{{"2", "5"}};
  correct_config[bear_server] = std::vector<Shard>{{"A", "Z"}};
  ASSERT(test_move(sm, bear_server, std::vector<Shard>{{"A", "Z"}}));
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);
