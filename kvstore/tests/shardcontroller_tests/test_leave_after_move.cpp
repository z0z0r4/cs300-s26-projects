...
int main() {
  ...
  If you pass test_complex_moves and fail this test, you have a problem in
  your Leave() shard reassignment logic.
  */
  ...
  for (std::size_t i = 0; i < N_SERVERS; i++) {
    ...
    correct_config[server_addresses[i]] = std::vector<Shard>{shards[i]};
  }
  ...
  correct_config[server_addresses[0]] = std::vector<Shard>{};
  correct_config[server_addresses[1]] = {{"0", "7"}, {"8", "E"}};
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);

  // Leave server_addresses[1]; server[1]'s shards should be moved to
  // server[0]
  ASSERT(test_leave(sm, server_addresses[1], true));
  correct_config.erase(server_addresses[1]);
  correct_config[server_addresses[0]] = {{"0", "7"}, {"8", "E"}};
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);
  ...
  // Leave server_addresses[2]; server[2]'s shards should be moved to
  // server[0]
  ...
  correct_config[server_addresses[3]] = std::vector<Shard>{};
  correct_config[server_addresses[4]] = {{"1", "Y"}, {"Z", "Z"}};
  ASSERT_EQ_CONFIGS(query_config(sm), correct_config);

  // Leave server_addresses[0]; server[0]'s shards should be moved to
  // server[3]
  ...
  // Leave server_addresses[3]; server[3]'s shards should be moved to
  // server[4]
